...
import {
    ...
    ProductVariantListResponseFromJSON,
    ProductVariantListResponseToJSON,
...
export class OrderProductsApi extends runtime.BaseAPI {
    ...
    /**
     * Creates request options for listProductVariants without sending the request
     */
    async listProductVariantsRequestOpts(requestParameters: ListProductVariantsRequest): Promise<runtime.RequestOpts> {
        if (requestParameters['id'] == null) {
            throw new runtime.RequiredError(
                'id',
                'Required parameter "id" was null or undefined when calling listProductVariants().'
            );
        }
        ...
        let urlPath = `/v1/order/products/{id}/variants`;
        urlPath = urlPath.replace(`{${"id"}}`, encodeURIComponent(String(requestParameters['id'])));
     ...
     */
    async listProductVariantsRaw(requestParameters: ListProductVariantsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<ProductVariantListResponse>> {
        const requestOptions = await this.listProductVariantsRequestOpts(requestParameters);
        const response = await this.request(requestOptions, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => ProductVariantListResponseFromJSON(jsonValue));
    }

    /**
     * List product variants by product ID
     */
    async listProductVariants(requestParameters: ListProductVariantsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<ProductVariantListResponse> {
