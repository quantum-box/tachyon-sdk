...
# **verify_api_key**
> VerifyApiKeyResponse verify_api_key(verify_api_key_request)
...
with tachyon_sdk.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = tachyon_sdk.AuthAPIKeysApi(api_client)
    verify_api_key_request = tachyon_sdk.VerifyApiKeyRequest() # VerifyApiKeyRequest | 

    try:
        # Verify a public API key
        api_response = api_instance.verify_api_key(verify_api_key_request)
        print("The response of AuthAPIKeysApi->verify_api_key:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AuthAPIKeysApi->verify_api_key: %s\n" % e)
```
