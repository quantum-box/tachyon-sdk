...
pub trait SecretsApp: Debug + Send + Sync + 'static {
    async fn get_secret(&self, path: &SecretPath) -> AuthResult<Option<String>>;

    async fn put_secret(&self, path: &SecretPath, value: &str) -> AuthResult<()>;

    async fn delete_secret(&self, path: &SecretPath) -> AuthResult<()>;

    async fn list_secrets(&self, prefix: Option<&str>) -> AuthResult<Vec<SecretPath>>;
}
