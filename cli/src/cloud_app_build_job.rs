...
pub struct BuildWorkloadSpec {
    ...
    /// Wrangler configuration generated by the control plane.
    ///
    /// Optional so specs produced by control planes that predate the
    /// field still deserialize (the builder then behaves exactly as
    /// before: the repository supplies its own wrangler configuration).
    #[serde(default)]
    wrangler_config: Option<BuildWorkloadWranglerConfig>,
}

/// Wrangler configuration written into the app directory before the app's
/// own install and build commands run.
///
/// Like [`BuildWorkloadD1`], every policy decision is already resolved by
/// the control plane. The builder writes the document verbatim and decides
/// nothing about its content.
#[derive(Debug, Deserialize)]
struct BuildWorkloadWranglerConfig {
    file_name: String,
    contents: String,
}
...
async fn run_build(
    ...
    // Written before install and build so the framework's own build reads
    // it, the same way it would read a checked-in wrangler configuration.
    write_generated_wrangler_config(workload, &app_dir)?;
...
/// Write the control-plane generated wrangler configuration, if any.
///
/// A repository that opted in carries no wrangler configuration of its own,
...
fn write_generated_wrangler_config(workload: &BuildWorkloadSpec, app_dir: &Path) -> Result<()> {
    ...
    for existing in ["wrangler.toml", "wrangler.json", "wrangler.jsonc"] {
        if app_dir.join(existing).exists() {
            return Err(anyhow!(
                "the manifest generates wrangler configuration but {} is \
                 checked in at {}; remove it or unset worker.generateConfig",
                existing,
                app_dir.display()
            ));
        }
    }
    let path = app_dir.join(&config.file_name);
    tracing::info!(
        path = %path.display(),
        "writing control plane generated wrangler configuration"
    );
    std::fs::write(&path, config.contents.as_bytes())
        .context("failed to write the generated wrangler configuration")?;
    Ok(())
}
...
mod tests {
    ...
    /// install and build run, because the framework build reads it.
    #[test]
    fn generated_wrangler_config_is_written_into_the_app_directory() {
        let temp = tempfile::tempdir().unwrap();
        let mut workload = test_workload(Some("cloudflare_workers"));
        workload.wrangler_config = Some(BuildWorkloadWranglerConfig {
            file_name: "wrangler.json".to_string(),
            contents: "{\n  \"name\": \"demo\"\n}".to_string(),
        });

        write_generated_wrangler_config(&workload, temp.path()).unwrap();

        let written = std::fs::read_to_string(temp.path().join("wrangler.json")).unwrap();
        assert!(written.contains("\"name\": \"demo\""));
    }

    /// Two owners of the same file would make which one `wrangler deploy`
    /// reads depend on the file layout, so the build stops instead.
    #[test]
    fn a_checked_in_wrangler_config_rejects_the_generated_one() {
        let temp = tempfile::tempdir().unwrap();
        ...
        let error = write_generated_wrangler_config(&workload, temp.path()).unwrap_err();

        assert!(error.to_string().contains("wrangler.jsonc"), "got: {error}");
        assert!(!temp.path().join("wrangler.json").exists());
    }

    /// A workload without the field must behave exactly as before, so a
    /// repository that supplies its own configuration is untouched.
    #[test]
    fn a_workload_without_generated_config_writes_nothing() {
        let temp = tempfile::tempdir().unwrap();
        let workload = test_workload(Some("cloudflare_workers"));
